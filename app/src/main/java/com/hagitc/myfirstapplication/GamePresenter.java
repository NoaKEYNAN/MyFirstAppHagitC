...
{
    //מטרתה לתפקד כמתווכת בין הממשק המשתמש (המיוצג ב `BoardGame`)
    // ובין לוגיקת המשחק עצמה (המיוצגת ב `GameLogic`).
    // מחלקה זו מקבלת את הפעולות שהמשתמש רוצה לבצע (כמו לחיצה על משבצת בלוח המשחק) ומנהלת את התהליך הלוגי של המשחק בהתאם לפעולות של המשתמש- בדיקת תקינות המהלך,
    // עדכון הלוח, בדיקת ניצחון, והצגת הודעות למשתמש על סיום המשחק או על פעולות שגויות.
    // בכך, המחלקה מאפשרת פיקוד וניהול המשחק בצורה יעילה ומסודרת.
